...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Move()
    {
        ...
        Vector2 Normal = new Vector2(GroundNormal().x, GroundNormal().y);
        if (Input.GetKeyDown(dash))
        {
            StartCoroutine(Dash());
        }
        if (IsGrounded() && !IsDash && Input.GetAxisRaw("Horizontal") != 0 && rb.linearVelocityY < 1f && !IsSlamming)
        {
            anim.speed = 1;
            if(Normal != Vector2.up && !isJumping)
            {

                Vector2 TargetVelocity;
                Vector2 SlopeDirection = new Vector2(Normal.y, -Normal.x);
                TargetVelocity = SlopeDirection * Input.GetAxisRaw("Horizontal") * GroundSpeed;
    ...
    private void Jump()
    {
        ...
        if (Input.GetKeyUp(jump) && !IsDash && !IsGrounded())
        {
            if(rb.linearVelocity.y > 0f && !boosting)
            {
    ...
    {
        if (CanDash && !Input.GetKey(KeyCode.UpArrow) && !po.carrying)
        {
        ...
        else if(CanDash && Input.GetKey(KeyCode.UpArrow) && !po.carrying)
        {
            StartCoroutine(flicker(new Color(0.7215686f, 0.4705882f, 0, 1), new Color(1, 1, 1, 1)));
            CanDash = false;
            boosting = true;
    ...
    {
        RaycastHit2D hit = Physics2D.Raycast(new Vector2(feet.bounds.center.x, feet.bounds.min.y + 0.05f), Vector2.down, 0.2f, Ground);
        return hit ? hit.normal : Vector2.up;
    }

    public Transform GetGround()
    {
        RaycastHit2D hit = Physics2D.BoxCast(feet.bounds.center, feet.bounds.size, 0f, Vector2.down, .1f, Ground);
